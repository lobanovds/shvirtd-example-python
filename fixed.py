...
db_requests_table_name = os.environ.get('DB_REQUESTS_TABLE_NAME', 'requests')
...
async def lifespan(app: FastAPI):
    ...
    try:
        with get_db_connection() as db:
            ...
            CREATE TABLE IF NOT EXISTS {db_name}.{db_requests_table_name} (
            ...
            db.commit()
            print(f"Соединение с БД установлено и таблица '{db_requests_table_name}' готова к работе.")
            cursor.close()
    except mysql.connector.Error as err:
        print(f"Ошибка при подключении к БД или создании таблицы: {err}")
        exit(1)
        # Роняем приложение, иначе вместо рабочего приложения получаем кирпич после поднятия БД
        # TODO: Можно позаниматься фиксом кода, чтобы ошибка не влияла на работу приложения, но у меня нет времени
    yield
...
def index(request: Request, ip_address: Optional[str] = Depends(get_client_ip)):
    ...
    try:
        with get_db_connection() as db:
            cursor = db.cursor()
            query = f"INSERT INTO {db_requests_table_name} (request_date, request_ip) VALUES (%s, %s)"
            values = (current_time, final_ip)
            cursor.execute(query, values)
            db.commit()
            cursor.close()
    except mysql.connector.Error as err:
        # В случае, если мы подключились к БД при инициализации, когда в ней не было создано пользователя,
        # гарантированно получим ошибку "Таблица не найдена". Фикс exit(1) описал выше
...
def get_requests():
    ...
    try:
        with get_db_connection() as db:
            cursor = db.cursor()
            query = f"SELECT id, request_date, request_ip FROM {db_requests_table_name} ORDER BY id DESC LIMIT 50"
